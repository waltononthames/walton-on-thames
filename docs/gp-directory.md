...
- **Building versus entrance.** A listing's `lat`/`lng` is the building, with its source in `gp.building_point_source`. `gp.entrance` is separate and empty for all six until someone stands at the door. When it is filled, the practice map shows an "Entrance" marker and the caption stops saying the pin is not a door.
