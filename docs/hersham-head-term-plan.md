...
- [x] **1.2 hersham.org.uk redirect. Closed 5 September 2026 at the owner's instruction.** hersham.org.uk does reach the Hersham hub. **What is configured is not what this item specified, and the difference is recorded here so nobody assumes otherwise.** Checked 5 September 2026: every path collapses to the hub, so `hersham.org.uk/parakeets/` does not reach the parakeets page; the target is the www host, so it takes two hops rather than one; and `www.hersham.org.uk` still returns a Cloudflare 522 because it has no DNS record. The per-slug rules and the DNS fix are in `docs/hersham-org-uk-redirects.md` if this is ever revisited. **Do not print a deep hersham.org.uk URL on anything** until the path map exists.
  - Original specification: The domain is a Cloudflare zone (server header confirms it), so the redirect lives in the dashboard, not this repo. The exact rules are written up in `docs/hersham-org-uk-redirects.md` (prepared 4 September 2026, includes a paste-ready brief for Cloudflare's dashboard AI assistant). Darren applies them:
...
- [x] **3.4 Hersham in wartime. Done 5 September 2026, and the answer was no new page.** All three elements the item names were already covered or turned out not to be Hersham's. The 4 September 1940 Vickers raid happened at Brooklands in Weybridge and is already told on `/history/walton-in-wartime/`, correctly placed there. The industrial war work is already covered at length on `/hersham/hersham-lodge-and-hersham-place/`. No village war memorial could be verified. What was verifiable and genuinely new is a plaque inside St Peter's to Flying Officer M. C. F. Mee, lost with HMS Glorious in June 1940, so that was added to `/hersham/st-peters-church/`, where the plaque actually is.
  - Original specification: (entityType event). The 4 September 1940 Vickers raid (named in the extension section 4.2 but not built as its own page), the industrial estates' war work, Hersham's memorial. Sources per the Standards: Surrey History Centre, Imperial War Museum memorials register, CWGC.
- [x] **3.5 Weylands Farm and the drill plough. Done 6 September 2026: no page, and the claim itself was corrected.** The condition was sources beyond Wikipedia, and there are none. The sentence on Wikipedia carries **no citation at all**, and Wikishire reproduces it, so the two are one account rather than corroboration. Worse, the site was asserting it as fact in four places, and the checkable part is false: Jethro Tull had built a drill plough by about 1733, twenty-seven years before George III's reign began. Removed the assertion from the hub and the Victoria article, and rewrote the history hub's Royal glimpses section to report the story, refute the superlative and name George Greenwood's parish history as the place to settle it.
  - Original specification: (entityType event) if sources exist beyond Wikipedia; otherwise leave the sentence on the history hub as it is. Do not build a spoke on a single Tier 2 source.
- [x] **3.6 Hersham news stream. Started 6 September 2026.** The `news` schema had no `neighbourhood` field, so the monthly Hersham entry this item calls for, and the hub block in 2.6 that depends on it, were both impossible as written. Added it as an optional string so the five existing articles stay valid. First entry published: the four housing schemes registered with Elmbridge and awaiting a decision in Hersham, drawn from the council's own live planning data that this repo already fetches nightly. Reportage with no position taken, per the editorial stance. **This item stays open as a standing monthly commitment** rather than being finished by one article; 2.6 unlocks once three entries exist.
  - Original specification: Commit to at least one `news` entry per month with `neighbourhood: hersham`, drawn only from Tier 1 sources: Elmbridge council news and planning decisions (the "Shaping Hersham" programme and the Hersham Place Technology Park redevelopment already have council pages), Surrey CC library and highways notices, Walton & Hersham FC official announcements. Reportage, not campaigning: the development-and-planning page's editorial stance applies (report what is proposed and at what stage; take no position). This is what gives the hub and `/hersham/development-and-planning/` a freshness signal the competitor's three posts a year cannot match.
- [x] **3.7 Quarterly re-verification. First pass done 6 September 2026; recurs quarterly, so next due December 2026.** All 41 source URLs across the twelve Hersham articles were requested. **No dead links and no citation rot.** Six return 403 to automated clients, which is bot protection rather than absence, and the one apparent redirect (the Imperial War Museums page on Odette Sansom) proved to be a client artefact: a second client returns the correct article. **One factual gap was found and fixed:** the golf page described the Hersham Village course without mentioning that its whole site is the subject of application 2023/3519 for 221 homes, a GP surgery and a country park. Added with both the council record and the applicant's own consultation as sources. **Exactly one reviewedDate was bumped**, on the one file that changed, which is the rule this item sets.
  - Original specification: of every existing Hersham spoke: re-check each `sources` URL, update facts that changed, bump `reviewedDate` only when something actually changed (a `dateModified` bump with no change is a signal Google discounts and a claim the Standards forbid).
...
- [x] **6.1 Local organisations. Framework and first approaches drafted 7 September 2026.** See `docs/outreach/hersham/`. Four of this month's five are local organisations. Sending is Darren's. Targets: Hersham Community Group, Hersham Churches Together, Hersham Baptist Church, St Peter's Church, Hersham FC, Walton & Hersham FC (the fixtures feed we already carry is a natural reciprocal hook), Esher Rugby, Hersham Golf Club, Burhill Golf Club, Whiteley Village Trust, Hersham Library (Surrey CC), the Village Market, lovewalton.co.uk, Elmbridge Museum.
- [ ] **6.2 Topic pitches beyond the village.** Surrey Bird Club is in the September batch. The rest are logged in the outreach status table and weighted first for October, because a link from outside the village counts for more than one from inside it. Pitches: the parakeets page to bird and wildlife groups and the Surrey Bird Club; Sham 69 to punk history sites and the Walton Hop coverage; Queen Victoria's first steam train to railway history societies and the Royal Collection Trust's diary project; Whiteley Village to Arts and Crafts and almshouse heritage bodies; Hersham Lodge and Hersham Place to industrial-archaeology and motoring history groups. The extension's KPI stands: ten new referring domains in six months.
- [x] **6.3 Wikipedia. Talk note drafted 7 September 2026**, for Darren to post from his own account with the connection disclosed. It leads with the article's two stale external links, which is a service independent of us, and raises the uncited drill plough sentence. It does not ask for our links. Do not add links to our own site (conflict of interest, and it will be reverted). The article's two external links currently point to archived dead sites; the legitimate route is a note on the article's Talk page, disclosing the connection and suggesting the hub and specific sourced spokes as replacements, then leaving it to editors. Opus drafts the Talk note; Darren posts it **[Darren]**.
- [ ] **6.4 Local press.** A press calendar of dated hooks is in `docs/outreach/hersham/README.md`. Nothing sent, because the next hook due is the autumn parakeet roost. Surrey Live has a Hersham section. Pitch dated hooks: the anniversary of Victoria's diary entry each February, the Vickers raid each 4 September, the parakeet roost in autumn, and any Waitrose or Shaping Hersham milestone. Each pitch links to the spoke, and the spoke links to the hub.
